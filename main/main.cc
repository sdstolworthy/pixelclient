...
void wsEventHandler(void *args, esp_event_base_t base, int32_t eventId,
                    void *eventData) {
  ...
  case WEBSOCKET_EVENT_DATA:
    ...
    case 0x0: // Continue
      ...
      if (!ctx->buffer.append((uint8_t *)data->data_ptr, data->data_len)) {
        ESP_LOGE(TAG, "Image buffer overrun for %u bytes at offset %u",
      ...
      if (ctx->valid && data->fin) {
        ...
        std::unique_ptr<Image> img =
            std::make_unique<WebPImage>(std::make_unique<StaticImageSource>(
  ...
  if (digitalRead(TRIGGER_PIN) == LOW) {
    ...
    if (digitalRead(TRIGGER_PIN) == LOW) {
      ...
      if (digitalRead(TRIGGER_PIN) == LOW) {
        ...
        ESP_LOGI(TAG, "Erasing Config, restarting");
        wm.resetSettings();
        ESP.restart();
      }
      ...
      if (!wm.startConfigPortal("OnDemandAP", "password")) {
        ESP_LOGI(TAG, "failed to connect or hit timeout");
        delay(3000);
        // ESP.restart();
      } else {
...
void app_main(void) {
  ...
  if (!res) {
    ...
    // if you get here you have connected to the WiFi
    ESP_LOGE(TAG, "connected...yeey :)");
  }

  ESP_LOGI(TAG, "WiFi up");

  // Note - Config::instance() depends on WiFi being initialized already for the
  // RNG.
